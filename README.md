...
*   **上書き保護オプション**: 変換ファイルの出力先に同名のファイルが存在する場合の動作（上書きするか、連番を付加して別名で保存するかなど）を設定できます。
*   **個別変換機能**: リスト内の特定のファイルを選択し、そのファイルのみを変換・保存することができます。
...
3.  **(想定)** メインウィンドウにある「選択ファイル変換」ボタン（または同様の名称のボタン）をクリックします。
    *   *(実際のボタン名や操作方法に合わせて適宜修正してください。もし該当するボタンのスクリーンショットがあれば、ここに追加するとより分かりやすくなります。)*
4.  「変換後のファイルの保存先フォルダを選択してください」というダイアログが表示されます（一括変換時と同様）。変換されたファイルを保存するフォルダを選択し、「フォルダーの選択」をクリックします。
    *   **出力先フォルダ記憶**: このとき選択したフォルダも記憶され、次回、個別変換を行う際にデフォルトの保存先として提案されます。
